...
bool HelloWorld::init()
{
	...
	//
	dcount = get();
	count = Label::createWithTTF(std::to_string(dcount), "fonts/arial.ttf", 36);
	count->setPosition(Vec2(origin.x + visibleSize.width / 2,
		origin.y + visibleSize.height - count->getContentSize().height*2));
	this->addChild(count, 1);

	schedule(schedule_selector(HelloWorld::updateTime), 1.0f, kRepeatForever, 0);
	schedule(schedule_selector(HelloWorld::hitByMonster), 0.04f, kRepeatForever, 0);
	// 静态动画
	idle.reserve(1);
	...
	Btn_D->setName("Btn_D");
	auto Btn_X = MenuItemLabel::create(X);
	//Btn_W->setPosition(Vec2(origin.x + W->getWidth() * 2, origin.y + W->getHeight() * 3));
	Btn_X->setPosition(Vec2(origin.x + visibleSize.width - W->getContentSize().width, origin.y + W->getContentSize().height * 3));
	Btn_X->setName("Btn_X");
	auto Btn_Y = MenuItemLabel::create(Y, CC_CALLBACK_0(HelloWorld::AttackAction, this, Y));
	//Btn_W->setPosition(Vec2(origin.x + W->getWidth() * 2, origin.y + W->getHeight() * 3));
