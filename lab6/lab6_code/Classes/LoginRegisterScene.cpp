...
void LoginRegisterScene::loginButtonCallback(cocos2d::Ref * pSender) {
  // Your code here
	...
	request->setResponseCallback(CC_CALLBACK_2(LoginRegisterScene::onLoginResposeComplete, this));
...
void LoginRegisterScene::registerButtonCallback(Ref * pSender) {
  // Your code here
	...
	request->setResponseCallback(CC_CALLBACK_2(LoginRegisterScene::onRegisterResposeComplete, this));
	cocos2d::network::HttpClient::getInstance()->send(request);
	request->release();

}
void LoginRegisterScene::onLoginResposeComplete(HttpClient *sender, HttpResponse* response) {
...
void LoginRegisterScene::onRegisterResposeComplete(HttpClient *sender, HttpResponse* response) {
	auto buffer = response->getResponseData();
	rapidjson::Document doc;
	doc.Parse(buffer->data(), buffer->size());
	if (doc["status"] == true) {
		this->messageBox->setString(std::string("Register OK\n") + doc["msg"].GetString());
	}
	else {
